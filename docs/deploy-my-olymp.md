...
**Статику uvicorn не отдаёт.** FastAPI обслуживает только `/api`,
`/webhook` и `/health`; `GET /` вернёт `404`. Мини-приложение — это файлы
из `frontend/`, их должен отдавать веб-сервер перед приложением. Если в
кабинете MAX указан `https://my-olymp.ru`, а корень отвечает `404`,
приложение не откроется вовсе.

Что должен делать прокси перед uvicorn — ровно то, что описано в
[`deploy/nginx/my-olymp.ru.conf`](../deploy/nginx/my-olymp.ru.conf):
отдавать `frontend/` на `/` и проксировать `/api/`, `/webhook`, `/health`.
...
| `404` на `/` при работающем `/api` | статику никто не отдаёт: uvicorn обслуживает только API |
